...
class MT(nn.Module):
    ...
    def forward(self, x):
        z = []  # inference output
        za = []
        attn = [torch.empty((0))] * self.nl
        output = dict()
        for i in range(self.nl):
            attn_func = self.attn_m[i]
            attn[i] = attn_func.forward(x[0][i])  # conv
            ...
            m_func = self.m[i]
            x[0][i] = m_func.forward(x[0][i])  # conv
        ...
        bases_o = torch.cat([bases_o, torch.zeros(self.pad_size).to(x[0][0].device)])
        bases_o = bases_o.reshape(1, target, -1)
    ...
    def _make_grid(nx: int = 20, ny: int = 20):
        # yv, xv = torch.meshgrid([torch.arange(ny), torch.arange(nx)])
        xv = torch.arange(nx).repeat(ny, 1).float()
        yv = torch.arange(ny).repeat(nx, 1).transpose(0, 1).float()
        return torch.stack((xv, yv), 2).view((1, 1, ny, nx, 2)).float()
