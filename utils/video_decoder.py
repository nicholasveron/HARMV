...
class VideoDecoder:
    ...
    def __init__(self, path: str, bound: int) -> None:
        ...
        self.__video_container.open(path)
...
if __name__ == "__main__":
    import cv2
    import time
    SAMPLE_VIDEO: str = "/mnt/c/Skripsi/dataset-h264/R002A120/S018C001P008R002A120_rgb.mp4"
    decoder = VideoDecoder(SAMPLE_VIDEO, 1)
    fr_li: list[numpy.ndarray] = []
    fl_li_x: list[numpy.ndarray] = []
    fl_li_y: list[numpy.ndarray] = []
    start_timer = time.perf_counter_ns()
    while True:
        data: DecodedVideoData = decoder.read()
        if not data[0]:
            break
        fr_li.append(data[1])
        fl_li_x.append(data[2])
        fl_li_y.append(data[3])
    end_timer = time.perf_counter_ns()
    print(((end_timer-start_timer) / 1000000))

    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    res = decoder.get_resolution()

    writer = cv2.VideoWriter("test_flow.mp4", fourcc, 30, (res[1], res[0]*3))
    for fr, fl_x, fl_y in zip(fr_li, fl_li_x, fl_li_y):
        fl_x_s = numpy.dstack((fl_x, fl_x, fl_x))
        fl_y_s = numpy.dstack((fl_y, fl_y, fl_y))
        writer.write(numpy.vstack((fr,
                                   fl_x_s,
                                   fl_y_s)))
    writer.release()
